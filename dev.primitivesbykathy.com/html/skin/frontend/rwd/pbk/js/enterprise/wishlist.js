...
Enterprise.Wishlist.Widget.Form = Class.create(Enterprise.Widget, {
    ...
    initialize: function($super, action) {
        var _templateString = '<ul class="form-list">' +
                '<li><label for="wishlist-name">' + Translator.translate('Saved Items List Name') + '</label><div class="input-box"><input type="text" id="wishlist-name" maxlength="255" class="input-text required-entry validate-length maximum-length-255" name="name"/></div>' +
...
Enterprise.Wishlist.createWithCallback = function(createUrl, callback, useAjax) {
    ...
    if (!Enterprise.Wishlist.createWithCallbackDialog) {
        ...
        Enterprise.Wishlist.createWithCallbackDialog = new Enterprise.Widget.Dialog(
            Translator.translate('Create a New Saved Items List'),
...
function process_ajaxwishlist(url,button){
    ...
    if(url != ""){
        ...
        if(parts.length < 2){
            parts = url.split("fromcart");
            var url_q       = parts[0].split("wishlist");
            var base_url    = url_q[0];

            var second_part = parts[1].split("/");
            var productId   = second_part[2];
            var formKey     = jQuery("input[name='form_key']").first().val();
            var query       = second_part[3];
            in_cart_page = true;
        }else{
            var url_q       = parts[0].split("wishlist");
            var base_url    = url_q[0];
...
function construct_url(base_url,productId,formKey,query){
    if(in_cart_page){
        var url = base_url+"wishlistajax/wishlistajax/ajaxfromcart/item/"+productId+"/form_key/"+formKey+"/"+query;
    }else{
